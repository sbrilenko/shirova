...
/*$sql='SELECT id,client FROM portfolio ORDER BY client';
$db->query($sql);
$arr=$db->getArray();
foreach($arr as $index=>$val)
{
	$sql2='UPDATE portfolio SET client2=(SELECT id FROM partner_logo WHERE name="'.$val['client'].'") WHERE id='.$val['id'];
	print $sql2;
	$db->query($sql2);
}*/
...
echo "<div id='edit'>";
echo "<fieldset>";
echo "<legend align='center'>". (($item !== null) ? "Редактирование заказчика" : "Добавить заказчика") . "</legend>";
...
echo "<div class='help'>";
echo "только *.png <br />";
echo "размер ".$maxWidthPartnerLogoBig."x".$maxHeightPartnerLogoBig." px";
echo "</div>";
...
//------------------------------------
echo "<tr>";
echo "<td class='left'>Заказчик";

echo "<div class='help'>";
echo "Максимальная длина строки 32 символа";
echo "</div>";
