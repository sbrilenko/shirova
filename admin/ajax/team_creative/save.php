...
$qwery = "INSERT INTO ".$name_dir." 
            (name, position, weight, description) 
         VALUES 
            ('".$name."', '".$position."', '".$weight."', '".$description."')";
...
if (($tempFile = $_FILES['url_image_small']['tmp_name']) != "") {
    ...
    //   echo "<p style='color:red;'>Допустимое разрешение для  большого изобр. $maxWidthSmall x $maxHeightSmall px</p>";
    //   exit;
    //}       
...
if (($tempFile = $_FILES['url_image_down']['tmp_name']) != "") {
    ...
    //if ($image_info['width'] != $maxWidthDown || $image_info['height'] != $maxHeightDown) {
    //   echo "<p style='color:red;'>Допустимое разрешение для  большого изобр. $maxWidthDown x $maxHeightDown px</p>";
    //   exit;
    //}       
